...
function capitalizeNames() {
  ...
  const capitalList = items.map((item) => {
    return item.name.toUpperCase();
  });

  const capitalListSliced = capitalList.splice(1); // just showing I can splice
  return capitalListSliced;
}
...
function calculateTotalInventory() {
  ...
  let totalInventory = items.reduce((acc, obj) => {
    return acc + obj.inventory;
  }, 0);
...
function filterItemsByCategoryId(categoryId) {
  ...
  const foundCategoryItems = items.filter((item) => item.categoryId == categoryId);
...
function logCartItems() {
  ...
  items.forEach((item) => {
    let stringValueNumber = item.id.toString();

    if (idArr.includes(stringValueNumber)) {
      let indexValue = items.indexOf(item);
      console.log(`The List Index of ${item.name} is ${indexValue}`);
    }
  });
}
